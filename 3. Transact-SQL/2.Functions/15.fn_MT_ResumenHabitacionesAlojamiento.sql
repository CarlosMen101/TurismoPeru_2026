...
CREATE OR ALTER FUNCTION came.fn_MT_ResumenHabitacionesAlojamiento(
    @IdAlojamiento INT
)
...
BEGIN
	...
	SELECT
		...
		AVG(H.precio_noche),
		MAX(H.precio_noche)
	FROM came.habitacion H
		INNER JOIN came.tipo_habitacion TH ON TH.id_tipo_habitacion = H.id_tipo_habitacion
	WHERE
...
FROM came.fn_MT_ResumenHabitacionesAlojamiento(1);
GO
